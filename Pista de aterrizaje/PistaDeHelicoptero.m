...
clc
close all
vidObj = VideoReader('C:\Users\suare\Documents\MEGA\Documentos JD\Universidad\USTA\Ingenieria Electronica\Noveno semestre\Reconocimiento de patrones\Pista de aterrizaje\videoHeliPads.mov'); % open file
get(vidObj);
...
for iFrame=1:nFrames
    ...
    if iFrame > 1
        for n=1:size(propied,1)
            if propied(n).Area > 50
                delete(r(n))
                delete(c(n))
            end
        end
    end
    [L Ne] = bwlabel(snap_blue);
    propied = regionprops(L);  
    set(hImage,'CData',snap);
    for n=1:size(propied,1)
        if propied(n).Area > 50
            r(n) = rectangle('Position',propied(n).BoundingBox,'EdgeColor','g','LineWidth',2);
            c(n) = rectangle('Position',propied(n).BoundingBox,'Curvature',[1 1],'EdgeColor',[1 0 1]);
        end
    end
